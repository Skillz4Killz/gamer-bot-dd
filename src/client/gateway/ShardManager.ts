...
export class ShardManager extends Collection<number, Shard> {
  ...
  spawn(id: number) {
    console.log('SPAWNING', 1)
    let shard = this.get(id)
    
    if (!shard) {
      console.log('SPAWNING', 2)
      shard = new Shard(id, this.client)
      console.log('SPAWNING', 2.5)
      this.set(id, shard)
    ...
    console.log('SPAWNING', 3)
    if (shard.status === 'disconnected') {
      console.log('SPAWNING', 4)
