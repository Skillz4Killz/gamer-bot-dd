...
export class Client extends EventEmitter {
  ...
  constructor(token: string, options: ClientOptions) {
    ...
    this.options = {
      ...
      applicationId: options.applicationId ?? getBotIdFromToken(token),
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      for (let i = this.options.firstShardID; i <= this.options.lastShardID; ++i) {
        console.log('Connecting', 5, i)
        this.shards.spawn(i)
      }
    } catch (err) {
      if (!this.options.autoreconnect) throw err
  ...
  async makeRequest(data: RequestData) {
    return (await this.requestHandler.request(data.method, data.url, undefined, data.body, data.file as FileContent)) as any
  }
