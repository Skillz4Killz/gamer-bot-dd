...
import { createRestManager, RequestMethods, RestManager } from '../rest/manager.js'
...
export class RequestHandler {
  ...
  constructor(client: Client, options: RequestHandlerOptions) {
    ...
    const ddbase = options.baseURL ?? this.client.options.proxyURL
    this.discordeno = createRestManager({
      token: this.client.token,
      baseUrl: ddbase?.startsWith('https://') ? ddbase : undefined,
    })
  ...
  async request(method: RequestMethods, url: string, auth?: boolean, body?: any, file?: FileContent): Promise<unknown> {
    ...
    return await this.discordeno[method.toLowerCase()](`https://discord.com/api/v10${url}`, body)
  }
