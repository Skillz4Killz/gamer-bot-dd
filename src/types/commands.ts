...
import type { translationKeys } from '../utils/translate'

import english from '../languages/english.js'

type Channel = Camelize<DiscordChannel>
type Interaction = Camelize<DiscordInteraction>
type Member = Camelize<DiscordMember>
type Role = Camelize<DiscordRole>
type User = Camelize<DiscordUser>
...
export interface Command<T extends readonly ArgumentDefinition[]> {
  ...
  options?: T
  execute: (data: Interaction, args: ConvertArgumentDefinitionsToArgs<T>) => unknown
  ...
  permissionLevels?: (keyof typeof PermissionLevelHandlers)[] | ((data: Interaction, command: Command<T>) => boolean | Promise<boolean>)
...
> = {
  ...
  BotSupporter: () => false,
  BotDev: () => false,
  BotOwner: (payload) => ['130136895395987456', '615542460151496705'].includes(payload.member?.user.id ?? payload.user!.id),
}
